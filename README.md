...
- `enable_anythingllm_site` -- publish `storage/anythingllm-fs/site` (`diot:anythingllm`, setgid) as a tailnet-only website at `https://<host>.<tailnet>.ts.net:8443` (`server_apps_defaults.anythingllm.site_port`). The folder is inside the built-in File System skill's root, so AnythingLLM's agent writes the site itself, from chat or from a scheduled job, as `site/...`. A rootless nginx container (`nginxinc/nginx-unprivileged`, pinned to the 1.30 series, running as `diot` through `UserNS=keep-id:uid=101`) serves the folder read-only on `127.0.0.1:8088` (`site_local_port`), and `tailscale serve` proxies the tailnet port to it; the play switches the port over only after nginx answers with its headers, and replaces the older direct folder handler in one step. nginx's config lives in root-owned `/srv/anythingllm/site-nginx`, beyond the agent's reach, and sends `Cache-Control: no-cache` (so a browser never reuses yesterday's front page), a `script-src 'none'` Content-Security-Policy that blocks script on every page whatever the agent writes, `Referrer-Policy: no-referrer` and `nosniff`, with directory listings and dotfiles refused. On first install it places a `README.md` of house rules and an empty root `index.html` in the folder. The rules keep the root to those two files, with every publication in its own folder (the daily paper in `news/`) listed from the root index; build each publication's pages from one `template.html` and `style.css`; and never use a real newspaper's name or masthead. Neither file is ever overwritten, so you or the agent can change them from chat. The separate port keeps the site a different origin from AnythingLLM's login, and the play refuses to take over a port another listener already serves.

...
- `enable_anythingllm_site` -- publish `storage/anythingllm-fs/site` (`diot:anythingllm`, setgid) as a tailnet-only website with `tailscale serve` at `https://<host>.<tailnet>.ts.net:8443` (`server_apps_defaults.anythingllm.site_port`). The folder is inside the built-in File System skill's root, so AnythingLLM's agent writes the site itself, from chat or from a scheduled job, as `site/...`; no web server is added, and the separate port keeps its pages a different origin from AnythingLLM's login. On first install it places a `README.md` of house rules there: build every page from one `template.html` and `style.css`, keep a `script-src 'none'` Content-Security-Policy line first in every `<head>`, never use a real newspaper's name or masthead, and lay the site out as `index.html` plus `archive/`. It is never overwritten, so you or the agent can change the rules from chat. They hold only as far as the agent follows them: `tailscale serve` cannot send security headers of its own. The play refuses to take over a port another listener already serves.

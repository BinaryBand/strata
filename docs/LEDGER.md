...
**RESOLVED: Prerequisite registration** -- named prerequisites used to be registered at import time by `utils/prerequisites.py`, which meant `main.py` had to import that module for its side effect alone and every `@guard.prerequisite` carried a lazy import to guarantee it had happened. That module is gone; the registry is now a plain table (`_PREREQUISITES` in `adapters/guard_executor.py`) that needs neither.

**Playbook parameterization is inconsistent** -- `ensure_path.yml` is fully parameterized via extravars. Most other playbooks have hardcoded paths and image names.

- Opportunity: move more playbook-specific values to extravars for consistency and easier customization.
...
**`_path_satisfied()` is imperative** -- the ownership/permission check (`adapters/guard_executor.py`) builds up stat results and compares them in a chain of ifs. A small `PathSpec(owner, group, mode)` dataclass with an `is_satisfied(stat_result) -> bool` method would be more readable and testable.

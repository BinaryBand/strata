...
Unit tests mirror the package under `tests/unit/` (e.g. `tests/unit/core/test_guard.py` covers `strata/core/guard.py`). Cross-cutting gates sit at the top level: `test_lint.py` (the lint/type gate below), `test_guard_completeness.py` (every runbook's playbook exists, its guard chain provides the tools that playbook shells out to, and its owned paths are declared *after* the guard that creates the owner), `test_feature_bindings.py` (every `tests/features/*.feature` is either bound by a `scenarios(...)` call or explicitly tagged `@wip`; all eight are currently bound), `test_server_apps_defaults.py` (images and published ports are sourced from the `server_apps_defaults` group_var, every field they reference exists on the model, and no data directory is re-declared in a playbook's `vars:`), `test_runbook_naming.py` (the `install_*`/`enable_*` rule plus its allowlist), `test_alias_completeness.py` (every runbook carries a `@guard.alias` and no two aliases collide), `test_playbook_registers.py` (no playbook or role task file registers a variable name twice, since a skipped task still overwrites the name), and `test_wireguard_routing.py` (the WireGuard tunnel's policy rules carry explicit priorities that sit between Tailscale's block and `main`). The shared playbook paths and the task-tree walker those gates use live in `tests/_ansible.py`.
...
strata runbook services.install_jellyfin
  ...
  -> guard_executor.execute(module, target=..., reporter=TyperReporter())
      ...
      UpstreamRunbook("infrastructure.install_podman") -> its guards always;
                                               its main() only if check() says not done
...
Each requirement has a cheap local check that can skip the playbook (`check()`, `path_satisfied`). Those only make sense when the target *is* the controller, so they are gated on `is_controller(target)`, which asks the inventory whether the host is `ansible_connection=local`.

A system-user guard has no fast path at all. `create_diot_user.yml` guarantees five things -- the group, the user, a subuid range, a subgid range and lingering -- and the `pwd.getpwnam` lookup that used to stand in for it was evidence of exactly one, so a half-provisioned `diot` satisfied the guard permanently. And a satisfied `check()` skips only its own runbook's `main()`, never the guards that runbook declares: it answers for that runbook's work, and was never evidence about its dependencies.

- `strata/cli/` -- the `strata` CLI entrypoint (Typer). `main.py` assembles the app tree and owns only the top-level `runbook` command; each command group lives in its own module under `cli/commands/` (`config`, `rclone`, `device`, `dev`), added via `app.add_typer`. `dispatch.py` holds `run_runbook` (resolve name -> import module -> `guard_executor.execute`) and lives apart from `main.py` on purpose: `main` imports the command modules and they need to trigger runbooks, so keeping dispatch separate breaks that cycle. `gui_server.py` builds the read-only JSON snapshot the GUI app consumes and serves it as a loopback API (see "The GUI" below). `picker.py` the interactive autocomplete runbook picker (a single `questionary.autocomplete` over a flat list of all runbook dotted names, with each one-line summary shown as meta) used when `strata runbook` is called without a NAME on a tty (the project's only `questionary` import site; returns `None` when stdin isn't a terminal so scripts keep erroring instead of hanging), `_helpers.py` the shared UX helpers (`not_found`, `apply_hint`), `wiring.py` the composition root. Subcommands beyond `runbook`: `gui` (serve the GUI's API on loopback), `config var|secret|vault-password|key`, `rclone` (`add`/`remove`/`list` + nested `serve add|remove|list` and `sync add|remove|list`), `device` (remote inventory hosts in the `[remote]` group of `hosts.ini`), and the hidden `dev` group (`dev schema` regenerates `.vscode/server_apps_schema.json` from the `ServerAppsDefaults` model -- run after changing it; `dev gui-data` prints the GUI snapshot for inspection by hand; `dev` is registered `hidden=True` so it stays off the operator `--help`). `strata runbook --tags` is forwarded only to runbooks whose `main()` accepts a `tags` argument (currently `infrastructure.backup` and `infrastructure.restore`).
...
- `strata/adapters/guard_executor.py` -- satisfies declared requirements, then calls `main()`.
- `strata/adapters/prerequisites.py` -- the named prerequisites `@guard.prerequisite` refers to (`sudo_password`, `vault_password`). Each entry pairs the way to establish one with the way to tell whether it already holds, so the set is enumerated once: `guard_executor` calls `ensure()`, `guard_status` calls `satisfied()`. The read half used to be a second `if name == ...` ladder in `guard_status.py`, which would have reported any third prerequisite missing forever. This is a plain table, not the old `utils/prerequisites.py` runtime registry that had to be imported for its side effect.
...
- `ansible/inventory/host_vars/<host>.yml` -- per-host plain (unencrypted, committed) variables -- rclone remotes/serves, published SSH keys, the restic repository override -- written by `strata config var`/`strata config key`/rclone registration, each scoped to a `--target` host so no two hosts share a value. `ansible/inventory/group_vars/all/server_apps_defaults.yml` -- server-app images and published ports, hand-edited and validated against the `ServerAppsDefaults` Pydantic model (`strata/core/models/server_apps_config.py`, JSON schema via `strata dev schema`); this one is a genuine shared default, so it stays in `group_vars/all/`. `ansible/inventory/group_vars/all/restic.yml` -- the restic repo-path resolution, image and config tag, shared by `backup.yml`/`restore.yml`/`install_restic.yml` (the repo-path regex used to be copied into all three). `ansible/inventory/group_vars/secrets/all.yml` -- vault-encrypted secrets (gitignored). The vault password lives only in the OS keychain (service `strata`, account `vault`; migrated forward, once, from the pre-rename `mr-manager` service name), retrieved at playbook time by `ansible/vault_pass.py`.
...
- `static/` -- non-Python assets: `git_apps.toml` (pipx-installed local git apps; currently no entries, so `services.install_from_git` reports itself not installed and its `main()` returns 0 after saying there is nothing to do), `rules/` ast-grep lint rules, `homebrew-tools/` (the local tap consumed by `install_antigravity.yml`).
...
The GUI is a Flutter app over the runbook catalog, living in its own repository at `~/Dev/apps/strata`. It is a second consumer of `discovery`, `guard` and `inventory`, and it reaches them through one HTTP server: `strata gui` (`strata/cli/gui_server.py`), which serves the read-only snapshot and an action API that actually runs runbooks, manages devices and sets secrets. That API is all it serves -- no static files, no web bundle. The app is served by its own tooling and calls in from another origin, so every response carries CORS headers.
...
strata gui
  -> gui_server.serve()
      ...
      OPTIONS /api/*              -> preflight (Authorization, Content-Type)
      GET  /*                     -> 404 (no static files)
...
`GET /api/gui-data` is still **declarations only**: `discovery.iter_runbooks()` for the catalog and `guard.declared(main)` for each guard chain, plus `inventory.all_hosts()`. Every other route is a thin bridge in `strata/cli/gui_actions.py` onto something that already exists -- `post_device` is `inventory.add`, `post_secret` is `secrets.set_secret`, `post_run` spawns a background thread running the real `guard_executor.execute()` and buffers its reported lines (`QueueReporter`) for `GET /api/run/<id>` to poll. `strata/adapters/guard_status.py`'s `guard_status()` is the read-only counterpart to `guard_executor`'s prompting/mutating `_satisfy_one()`, used by `/api/runbook-status` to report each guard as `"satisfied"`/`"missing"`/`"unknown"` without ever prompting or running a playbook. That module also owns `check_result()`, the single gate deciding when a runbook's own `check()` is meaningful: every `check()` in the codebase reads the local filesystem, so it describes `target` only on the controller. `/api/runbook-status`'s `installed` is `null` off the controller and `null` for a runbook with no `check()` -- the key is always present, only its value goes null. It used to call `check_safely` with no gate at all while reporting each guard target-aware in the same response, so one payload described two different machines.

Because the action routes can run privileged playbooks, every mutating one (`POST`/`DELETE`) requires `Authorization: Bearer <token>` -- `strata/adapters/gui_token.py`, a keychain entry alongside the vault password (service `strata`, account `gui_token`). `strata gui` prints the token on startup; `strata dev gui-token` reprints (or `--rotate`s) it for pasting into a second device reached via `tailscale serve <port>` (never `tailscale funnel`) -- the server still binds `127.0.0.1` only, so the token is what stops another tailnet device from running a playbook against this box, not a replacement for that boundary.

A browser discards a cross-origin response that carries no `Access-Control-Allow-Origin`, so the server sets one: a loopback origin is echoed back whatever port it picked (`flutter run` chooses a fresh one per launch), and any other origin -- a device reached over `tailscale serve` -- has to be named with `strata gui --allow-origin <origin>`. Naming it is deliberate: the request's own `Origin` header is not evidence of anything. `strata dev gui-data` is the standalone read-only snapshot printer, for inspection by hand.

The snapshot and the action payloads are an untyped wire format shared with Dart, so `tests/unit/cli/test_gui_server.py`, `tests/unit/cli/test_gui_actions.py` and `tests/unit/adapters/test_guard_status.py` pin them: the exact key set of each object, and -- driven off the `Requirement` union -- that every requirement type has a label in `_GUARD_LABELERS`. Both sides degrade silently otherwise (Python falls back to the class name, Dart to `GuardType.requires`), so a new `@guard.*` needs a labeler here and a `GuardType` in the app's `lib/data/strata_cli.dart`.
...
No Dart lives in this repository and none is covered by `uv run pytest`. The GUI repository carries its own build instructions and its own gate.
...
- Guards resolve prerequisites idempotently: prefer a cheap local check first, and only do privileged work (usually running a small playbook) if that check fails. A fast path is legitimate only when it answers the *same* question the playbook does. `pwd.getpwnam("diot")` and `shutil.which("brew")` each answered a cheaper, different one -- "does the account exist" for a play that also allocates subuid/subgid ranges and lingering, and "is brew on PATH" for a play that installs it to a prefix on no PATH -- and both were wrong for as long as they existed. A check that cannot answer the playbook's question is worse than no check.
- A runbook's `check()` reports on its own `main()`, nothing more. It never speaks for the guards that runbook declares, and it is only meaningful on the controller, since every one of them reads the local filesystem.
...
- A server app's data directories belong to the runbook, not the playbook. The runbook spells each one once as a module constant, declares it with `guard.path` (and `guard.backup_tag` where it is snapshotted), and passes it to its playbook as an extravar; the playbook asserts it arrived and never declares it in `vars:`. The directory a guard provisions and the directory a container binds have to be the same object, and a literal in both places is two owners for one path. Images and published ports go the other way -- they are deployment config with no Python-side counterpart, so they live in `server_apps_defaults` and the playbook reads them from there. `tests/test_server_apps_defaults.py` enforces both directions.
- No source module may exceed 400 lines (`tests/test_lint.py::test_module_length`). Test code is exempt: a pytest-bdd binding is a flat catalogue of three-line step definitions whose natural unit is its `.feature` file.
- Every source module carrying logic needs its mirror test at `tests/unit/<path>/test_<name>.py` (`tests/test_lint.py::test_tests_mirror_package`). A package `__init__.py` maps to `test_<dir>.py`. Modules with no top-level `def`/`class` are skipped, as are `__main__.py`, `ports.py`, and everything under `runbooks/` and `models/` -- runbooks are covered by the feature suite instead. Adding a module outside those exemptions without its mirror fails the suite.

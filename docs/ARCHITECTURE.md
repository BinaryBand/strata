...
Unit tests mirror the package under `tests/unit/` (e.g. `tests/unit/core/test_guard.py` covers `strata/core/guard.py`). Cross-cutting gates sit at the top level: `test_lint.py` (the lint/type gate below), `test_guard_completeness.py` (every runbook's playbook exists, its guard chain provides the tools that playbook shells out to, and its owned paths are declared *after* the guard that creates the owner), `test_feature_bindings.py` (every `tests/features/*.feature` is either bound by a `scenarios(...)` call or explicitly tagged `@wip`; all eight are currently bound), `test_server_apps_ports.py`, `test_runbook_naming.py` (the `install_*`/`enable_*` rule plus its allowlist), `test_alias_completeness.py` (every runbook carries a `@guard.alias` and no two aliases collide), `test_playbook_registers.py` (no playbook or role task file registers a variable name twice, since a skipped task still overwrites the name), and `test_wireguard_routing.py` (the WireGuard tunnel's policy rules carry explicit priorities that sit between Tailscale's block and `main`). The shared playbook paths and the task-tree walker those gates use live in `tests/_ansible.py`.
